...
public class SectorModUtils
{
    ...
    public final synchronized void init(Context context)
    {
        if (_initCalled == false)
        {
            try
            {
                loadData(context, SymbolID.Version_2525Dch1);
                loadData(context, SymbolID.Version_2525Ech1);
                _initCalled = true;
     ...
     * @param context
     */
    private void loadData(Context context, int version)
    {
        String[] temp = null;
        String delimiter = "\t";
        int ver = 0;
        ...
        try {
            ...
            if(ver == SymbolID.Version_2525Dch1)
                is = context.getResources().openRawResource(R.raw.smd);
            else
                is = context.getResources().openRawResource(R.raw.sme);
    ...
    public String getName(int version, int symbolSet, int location, String code)
    {
        ...
        if(_sectorMods.containsKey(id))
            return _sectorMods.get(id);
        else
            return "";
    }
}
