...
public class MultiPointHandlerSVG {
    ...
    public static String GeoSVGize(String id, String name, String description, String symbolID, ArrayList<ShapeInfo> shapes, ArrayList<ShapeInfo> modifiers, IPointConversion ipc, boolean normalize, String textColor, String textBackgroundColor, boolean wasClipped, Rectangle bbox) {
        ...
        try {
            height = RendererSettings.getInstance().getMPLabelFontSize();
            ...
            for (int i = 0; i < len; i++) {
                ...
                if (svg != null) {
                    ...
                    if (pathBounds == null)
                        pathBounds = new Rectangle(tempBounds);
                    else
            ...
            for (int j = 0; j < len2; j++) {
                ...
                if (tempModifier.getModifierString() != null && !tempModifier.getModifierString().isEmpty()) {
                    ...
                    tiTemp = new SVGTextInfo(tempModifier.getModifierString(), tempLocation, strJustify, degrees);
                    ...
                    Bitmap imgModifier = tempModifier.getModifierImage();
                    ...
                    paths.add("<image transform=\"translate(" + x + ',' + y + ") rotate(" + angle + ")\" href=\"" + MultiPointHandler.bitmapToString(tempModifier.getModifierImage()) + "\" />");
                    if (angle != 0) {
                        bounds = SVGTextInfo.getRotatedRectangleBounds(bounds, tempLocation, -angle, "middle");
                    }
                    if (bounds != null) {
                        if ((bbox != null && bbox.intersects(bounds)) || bbox == null) {
                            if (pathBounds != null)
            ...
            if (pathBounds != null) {
                unionBounds = new Rectangle(pathBounds);
            }
            if (labelBounds != null) {
                if (unionBounds != null) {
    ...
    static String getFillPattern(ShapeInfo shapeInfo) {
        if (shapeInfo.getPatternFillImage() != null) {
            ...
            return "<defs><pattern id=\"fillPattern\" patternUnits=\"userSpaceOnUse\" width=\"" + width + "\" height=\"" + height + "\"><image href=\"" + MultiPointHandler.bitmapToString(shapeInfo.getPatternFillImage()) + "\" /></pattern></defs>";
        } else {
            return null;
        }
    }
    ...
    static String ShapesToGeoSVG(String symbolID, ShapeInfo shapeInfo, Rectangle pathBounds, IPointConversion ipc, boolean normalize) {
        ...
        pathBounds.setRect(new Rectangle(path.getBounds()));
        return svgElement;
    }
}