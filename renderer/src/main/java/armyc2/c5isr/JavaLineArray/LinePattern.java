...
public class LinePattern {
    ...
    private static final LruCache<String,LinePattern> _patternCache = new LruCache<>(128);
    Bitmap pattern = null;
    String svg = null;
    double vOffset = 0;

    LinePattern(Bitmap linePattern, String svgPattern, double verticalOffset)
    {
        pattern = linePattern;
        svg = svgPattern;
    ...
    public Bitmap getLinePatternImage()
    {
        return pattern;
    }
    ...
    public static LinePattern getLinePattern(String symbolCode, Color lineColor, Color fillColor, float lineWidth)
    {
        ...
        if(symbolCode != null && symbolCode.length() >= 20)
        {
            ...
            {
                lp = (LinePattern)_patternCache.get(key);
                if(lp != null)
                    return lp;
            }
            ...
            if(ec == 140100)//FLOT
            {
                ...
                svgBounds = RectUtilities.makeRectF((float)left,(float)top,(float)width,(float)(bottom - top));
            }
            ...
            if(svgPath != null)
            {
                ...
                sbSVG.append("width=\"").append(svgBounds.width()).append("\" height=\"").append(svgBounds.height()).append("\" ");
                sbSVG.append("viewBox=\"").append(svgBounds.left).append(" ").append(svgBounds.top).append(" ").append(svgBounds.width()).append(" ").append(svgBounds.height()).append("\" ");
                ...
                //convert to image
                Rect biBounds = RectUtilities.makeRect(svgBounds.left,svgBounds.top,svgBounds.width(),svgBounds.height());
                Bitmap bmp = RendererUtilities.renderSVG(biBounds, sbSVG.toString());
    ...
    {
        Bitmap original = linePattern.getLinePatternImage();
        ...
        Canvas ctx = new Canvas(result);

        ctx.drawBitmap(result,0,drawY,null);
